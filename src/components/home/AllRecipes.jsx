       ...
       >
        ...
        <div className="flex space-x-4" key={index}>
          ...
          <div className="flex flex-col space-y-1">
            <Link rel="noopener noreferrer" className="text-sm font-semibold">{recipe?.userDetails?.name}</Link>
            {/* <span className="text-xs dark:text-gray-400">4 hours ago</span> */}
          </div>
        </div>
        <Link to={`/recipe/${recipe.id}`}>
        <div>
          <img src={recipe.image} alt="" className="object-cover w-full mb-4 h-50 sm:h-96 dark:bg-gray-500" />
          <h2 className="mb-1 text-xl font-semibold">{recipe.name}</h2>
          <p className="text-sm dark:text-gray-400">{truncateText(recipe?.description, 30)}</p>
        </div>
          ...
          <div className="flex space-x-2 text-sm dark:text-gray-400">
            ...
            <button type="button" className="flex items-center p-1 space-x-1.5">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" aria-label="Number of likes" className="w-4 h-4 fill-current dark:text-violet-400">
                ...
                <path d="M494.459,277.284l-22.09-58.906a24.315,24.315,0,0,0-22.662-15.706H332V173.137l9.573-21.2A88.117,88.117,0,0,0,296.772,35.025a24.3,24.3,0,0,0-31.767,12.1L184.693,222.937V248h23.731L290.7,67.882a56.141,56.141,0,0,1,21.711,70.885l-10.991,24.341L300,169.692v48.98l16,16H444.3L464,287.2v9.272L396.012,415.962H271.07l-86.377-50.67v37.1L256.7,444.633a24.222,24.222,0,0,0,12.25,3.329h131.6a24.246,24.246,0,0,0,21.035-12.234L492.835,310.5A24.26,24.26,0,0,0,494.459,277.284Z"></path>
              </svg>
